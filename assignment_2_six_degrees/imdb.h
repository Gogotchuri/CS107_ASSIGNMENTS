...
private:
  ...
  /*General function to search and if exists get address of entity with 'name' in 'file';
    ...
    @param name name/title of entity to search for;
    @param file movieFile/ActorFile(or file with same structure) to look inside
    @param offset_addr getting address to entity offset number(in case we need next or previous element)
                      (if no need just pass NULL ptr)
  */
  void *get_addr(const string name, const void *file, void **offset_addr) const;
