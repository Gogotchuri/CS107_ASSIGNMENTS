...
{
  pthread_mutex_t *mtx1 = &(bank->transfer_rep_lock);
  pthread_mutex_t *mtx2 = &(bank->report_compare_lock);
...
void reset_day(Bank *bank, int n)
{
  for (int i = 0; i < n - 1; i++)
    sem_post(&(bank->starting_new));

  pthread_mutex_unlock(&(bank->report_lock));
  bank->counter = n;
}
int Report_DoReport(Bank *bank, int workerNum)
{
  pthread_mutex_t *mtx = &(bank->report_lock);
  pthread_mutex_t *mtx2 = &(bank->report_compare_lock);
  ...
  if (bank->counter == 1)
  {
    if (rpt->numReports >= MAX_NUM_REPORTS)
    {
      // We've run out of report storage for the bank
      pthread_mutex_unlock(mtx2);
      reset_day(bank, bank->num_workers);
      return -1;
    }
...
{
  pthread_mutex_t *mtx1 = &(bank1->report_compare_lock);
  pthread_mutex_t *mtx2 = &(bank2->report_compare_lock);
